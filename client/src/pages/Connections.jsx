...
import { Users, UserPlus, UserCheck, UserRoundPen, MessageSquare, icons, User } from 'lucide-react'
      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8 flex flex-wrap gap-6'>
          {dataArray.map((item, index) => (
            <div key={index} className='flex flex-col items-center justify-center gap-1 border h-20 w-40 border-gray-200 bg-white shadow rounded-md p-6'>
        ...
        <div className='inline-flex flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm '>
          {dataArray.map((tab) => (
            <button onClick={() => setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm rounded-md transition-colors cursor-pointer ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
              ...
              {tab.count !== undefined && (
                <span className='ml-1 text-xs bg-gray-100 font-medium text-gray-700 px-2 py-0.5 rounded-full'>{tab.count}</span>
              )}
            </button>
          ))}
        </div>
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  ...
                  }
                  {
                    currentTab === 'Followings' && (
                      <button className='w-full p-2 text-sm rounded bg-slate-200 hover:bg-slate-100 text-black active:scale-95 transition cursor-pointer'>
                        Unfollow
                      </button>
                    )
                  }
                  ...
                  }
                  {
                    currentTab === 'Connections' && (
                      <button onClick={() => navigate(`/messages/${user._id}`)} className='w-full p-2 text-sm rounded bg-slate-200 hover:bg-slate-100 text-slate-800 active:scale-95 transition cursor-pointer flex items-center justify-center gap-1'>
